...
class ExpertReplay:
    def __init__(
        ...
        n = min(len(self.qpos), len(self.actions))
        self.start_idx = max(0, int(start_idx))
        self.end_idx = n if end_idx is None else min(n, int(end_idx))
        if self.start_idx >= self.end_idx:
            raise ValueError(
                f"Invalid expert replay range [{self.start_idx}, {self.end_idx}) for {path}"
            )
...
def main():
    ...
    if args.dummy:
        ...
        rospy.init_node(
            "ros_bridge_original_act_qpos_10hz_binary_adaptation", anonymous=True
        )
        if adapt_cfg.enabled:
