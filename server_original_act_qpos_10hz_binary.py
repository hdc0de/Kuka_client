"""Inference server for the original-ACT 10 Hz qpos binary dataset."""
...
def load_policy_from_config(config: dict):
    ...
    if policy_name == "act_original_qpos_10hz_binary":
        from policies.original_act_qpos_10hz_binary_policy import (
            QPos10HzBinaryOriginalACTPolicy,
        )

        server_cfg = config["server"]
        policy = QPos10HzBinaryOriginalACTPolicy(
            ...
            camera_names=server_cfg.get("act_camera_names", ["cam_front", "cam_wrist"]),
            camera_sources=server_cfg.get("act_camera_sources", ["front", "wrist"]),
            ...
            gripper_closed_width_threshold=server_cfg.get(
                "act_gripper_closed_width_threshold", 0.068
            ),
            binary_action_threshold=server_cfg.get(
    ...
    raise ValueError(
        ...
        "Available: random, go_home, act, "
        "act_original_qpos_10hz_binary, pi05, pi05_h50, dp"
    )
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config_original_act_qpos_10hz_binary.yaml",
    )
